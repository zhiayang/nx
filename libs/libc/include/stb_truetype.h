		...
		if (id == target_id)
		{
			...
			if (platform == 0 || (platform == 3 && encoding == 1) || (platform == 3 && encoding == 10))
			{
				...
				if (matchlen >= 0)
				{
					...
					{
						stbtt_int32 slen_ = ttUSHORT(fc+loc+12+8);
						stbtt_int32 off_ = ttUSHORT(fc+loc+12+10);
						if (slen_ == 0)
						{
							if (matchlen == nlen)
							return 1;
					...
					{
						++matchlen;
						if (stbtt_CompareUTF8toUTF16_bigendian((char*) (name+matchlen), nlen-matchlen, (char*)(fc+stringOffset+off_),slen_))
						return 1;
				}
			} else
			{
